...
// Generisk: sand hvis fejlen skyldes en manglende kolonne (uanset hvilken).
// Bruges til valgfrie kolonner (source, consent_at) så en tilmelding kan gemmes
// uden dem, hvis migrationen ikke er kørt endnu.
export function manglerKolonne(err) {
  if (err?.code === "PGRST204" || err?.code === "42703") return true;
  const besked = err?.message ?? "";
  return /(schema cache|does not exist|could not find)/i.test(besked) && /column/i.test(besked);
}
