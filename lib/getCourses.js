...
// courses_cms i Supabase har ingen duration-kolonne. Varighed holdes derfor
// ét sted (siteData) og flettes ind via slug, så en ændring dér slår igennem
// på både kursusoversigt og detaljesider.
const DURATION_BY_SLUG = Object.fromEntries(
  [...courses, ...experiences].map((c) => [c.slug, c.duration])
);

function normalize(item) {
  return {
    ...item,
    duration:     item.duration ?? DURATION_BY_SLUG[item.slug] ?? "",
...
export async function getCourses() {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .neq("is_published", false)        // vis kun publicerede (true eller null/ikke sat)
...
export async function getExperiences() {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .neq("is_published", false)        // vis kun publicerede
      .order("sort_order", { ascending: true });
