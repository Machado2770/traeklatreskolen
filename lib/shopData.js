...
// Badge vist på varekort og produktside.
// Et manuelt `badge`-felt på varen vinder; ellers udledes badgen af kategorien,
// så den virker uanset om data kommer fra Supabase eller denne fallback.
// Vil du give en enkelt vare et badge manuelt, så tilføj fx badge: "Nyhed".
export function productBadge(p) {
  if (!p) return null;
  if (p.badge) return p.badge;
  if (p.category === "Klatrepakker") return "Pakketilbud";
  if (p.category === "T-shirts climbers collection") return "Nyhed";
  return null;
}

// Farve pr. badge-type (sidens grøn/orange-palet).
export const BADGE_COLORS = {
  Nyhed: "#216344",
  Pakketilbud: "#d8782f",
  "Bruges i undervisningen": "#1f3a2b",
};
...
export const products = [
  ...
  },
  {
    ...
    short: "Stor HMS-karabin med skruelås — robust og rummelig.",
    price: 129,
    category: "Karabiner",
    image: "/images/shop/wild-country-titan-ii.jpg",
    description:
      "Stor, pæreformet HMS-karabin med skruelås. HMS-formen giver god plads til HMS-knob, rebbremser og flere slynger på én gang, og den brede åbning gør den nem at klippe i. Robust konstruktion til hård, daglig brug.",
    bullets: [
      "HMS-form — plads til knob og bremser",
      "Skruelås",
      "Stor åbning — nem at klippe",
      "CE-godkendt",
    ],
  },
  {
  ...
  // (Softshell, skaljakke og vest med logo er internt medarbejdertøj —
  //  se /admin/arbejdstoej. De sælges ikke i shoppen.)

  // ── T-shirts ────────────────────────────────────────────────────────────
  // Tekst-t-shirts med glimt i øjet. Størrelse vælges via sizes-feltet.
  {
    ...
    bullets: [
      ...
      "Unisex pasform",
      "Fås i S–XXL — vælg størrelse ved køb",
    ],
  },
  {
